...
Flow per model:
  Phase 1 — Generation
    example 1 → generate → save raw result
    example 2 → generate → save raw result
    ...all examples done, timing recorded for each
  Phase 2 — Scoring (batch, after ALL generation is complete)
    all generated texts scored together in one BERTScore pass
    results file updated with metrics

Usage:
    python benchmark_qna.py --model qwen3.5:4b     # single model
    python benchmark_qna.py --all                  # every model in models.csv
    python benchmark_qna.py --all --no-metrics     # timing only, skip scoring
"""
...
SYSTEM_PROMPT = (
    "Tu es un assistant administratif. Réponds à la question en te basant "
    "uniquement sur le contexte fourni. Réponds de manière concise et précise, "
    "en français."
)

DATA_PATH  = "data/qna_ground_truth.json"
RESULTS_DIR = "results/qna"
TASK_NAME  = "qna"


def run_qna_benchmark(model_tag: str, model_name: str, dataset: list,
                      compute_quality_metrics: bool = True):
    print(f"\n{'='*60}")
    print(f"QnA Benchmark — {model_name} ({model_tag})")
    print(f"{'='*60}")

    examples = []

    # ── Phase 1: generate every example, record timing ───────────────────────
    print("\n  Phase 1 — Generation")
    for item in dataset:
        prompt = f"Contexte :\n{item['context']}\n\nQuestion : {item['query']}"
        print(f"    [{item['id']}] generating...", end=" ", flush=True)

        result = call_ollama(model_tag, prompt, system=SYSTEM_PROMPT)
    ...
    if compute_quality_metrics:
        ...
        print("  (This runs once for all examples together — faster than one-by-one)")

        # Only score examples that generated successfully
        valid_indices = [
        ...
        # Update the saved file with metrics now included
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark LLMs on QnA task")
    parser.add_argument("--model", type=str,
                        help="Ollama tag of a single model, e.g. qwen3.5:4b")
    ...
    if args.all:
        for m in load_models_csv():
            run_qna_benchmark(m["ollama_tag"], m["model_name"], dataset, compute)
    elif args.model:
        run_qna_benchmark(args.model, args.model, dataset, compute)
    else:
