...
Flow per model:
  Phase 1 — Generation
    text 1 → generate → save raw result
    text 2 → generate → save raw result
    ...all texts done, timing recorded for each
  Phase 2 — Scoring (batch, after ALL generation is complete)
    all generated summaries scored together in one BERTScore pass
    results file updated with metrics

Usage:
    python benchmark_summary.py --model qwen3.5:4b
    python benchmark_summary.py --all
    python benchmark_summary.py --all --no-metrics
"""
...
SYSTEM_PROMPT = (
    "Tu es un assistant administratif. Résume le texte suivant en français, "
    "de manière concise et fidèle, en conservant les informations clés "
    "(dates, chiffres, décisions, responsables). Le résumé doit faire environ "
    "le quart de la longueur du texte original."
)

DATA_PATH   = "data/summary_ground_truth.json"
RESULTS_DIR = "results/summary"
TASK_NAME   = "summary"


def run_summary_benchmark(model_tag: str, model_name: str, dataset: list,
                          compute_quality_metrics: bool = True):
    print(f"\n{'='*60}")
    print(f"Summary Benchmark — {model_name} ({model_tag})")
    print(f"{'='*60}")

    examples = []

    # ── Phase 1: generate every summary, record timing ───────────────────────
    print("\n  Phase 1 — Generation")
    for item in dataset:
    ...
    if compute_quality_metrics:
        ...
        print("  (This runs once for all summaries together — faster than one-by-one)")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark LLMs on summarization task")
    ...
    if args.all:
        for m in load_models_csv():
            run_summary_benchmark(m["ollama_tag"], m["model_name"], dataset, compute)
    elif args.model:
        run_summary_benchmark(args.model, args.model, dataset, compute)
    else:
